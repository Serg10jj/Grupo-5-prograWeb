...
const Consulta = ({ consulta, refreshData }) => {
  const [edit, setEdit] = useState(false);
  const [libroId, setlibroId] = useState(consulta.libro_id);
  const [nombreLibro, setNombreLibro] = useState(consulta.nombre_libro);
  const [nombreUsuario, setNombreUsuario] = useState(consulta.nombre_usuario);
