...
const Book = ({ book, refreshData }) => {
  const [edit, setEdit] = useState(false);
  const [userId, setUserId] = useState(book.user_id);
  const [userEmail, setUserEmail] = useState(book.user_email);
  const [userName, setUserName] = useState(book.user_name);
  const [userLastName, setUserLastName] = useState(book.user_last_name);
  const [userPassword, setUserPassword] = useState(book.user_password);
